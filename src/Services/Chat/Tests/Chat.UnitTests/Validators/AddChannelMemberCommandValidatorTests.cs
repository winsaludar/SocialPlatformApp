...
public class AddChannelMemberCommandValidatorTests
{
    private readonly Mock<IRepositoryManager> _mockRepositoryManager;
    private readonly AddChannelMemberCommandValidator _validator;

    public AddChannelMemberCommandValidatorTests()
    {
        ...
        _validator = new AddChannelMemberCommandValidator(_mockRepositoryManager.Object);
    }
    ...
    public async Task UserId_IsInvalid_ThrowsUserNotFoundException()
    {
        // Arrange
        Server targetServer = GetTargetServer();
        AddChannelMemberCommand command = new(targetServer, Guid.NewGuid(), Guid.NewGuid(), targetServer.CreatedById);
        _mockRepositoryManager.Setup(x => x.ServerRepository.GetByIdAsync(It.IsAny<Guid>())).ReturnsAsync(targetServer);
    ...
    [Fact]
    public async Task AddedById_IsNotTheCreatorOrAModerator_ThrowsUnauthorizedUserException()
    {
        // Arrange
        Guid channelId = Guid.NewGuid();
        Guid userId = Guid.NewGuid();
        Server targetServer = GetTargetServer();
        AddChannelMemberCommand command = new(targetServer, channelId, userId, Guid.NewGuid());
        _mockRepositoryManager.Setup(x => x.ServerRepository.GetByIdAsync(It.IsAny<Guid>())).ReturnsAsync(targetServer);
    ...
    [Fact]
    public async Task User_IsAlreadyAMemberOfTheChannel_ThrowsUserIsAlreadyAMemberException()
    {
        // Arrange
        Guid channelId = Guid.NewGuid();
        Guid userId = Guid.NewGuid();
